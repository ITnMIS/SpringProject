	...
	function moveAction(where){
		...
		case 1:
			if(confirm("글을 삭제하시겠습니까?")){
				location.href ="delete.do?idx=${qna.idx}";
			}
			break;

		case 2:
			if(confirm("글을 수정하시겠습니까?")){
				location.href = "modify.do?idx=${qna.idx}";
			}
			break;
		...
		case 4:
			if(confirm("글을 추천하시겠습니까?")){
				location.href = "recommend.do?idx=${qna.idx}";
			}
			break;
		}
	}
...
<div class="wrapper">	
	<table class="boardView">
		<tr>
			<td colspan="4"><h3>${qna.subject}</h3></td>
		</tr>
		<tr>
			<th>작성자</th>
			<th>조회수</th>
		...
		<c:if test="${qna.fileupload != null }">
		<tr>
			<td colspan="4" align="left"><span class="date">첨부파일:&nbsp;<a href="<%=request.getContextPath()%>/files/${qna.fileupload}" target="_blank">${qna.fileupload}</a></span></td>
		</tr>
		</c:if>	
		<tr>
			<td colspan="4" align="left"><p>${qna.content}</p><br /><br /></td>
		</tr>		
	</table>
		...
		<tr>
			<td class="writer">				
				...
				<c:if test="${comment.writerId == id}">
					<br /><a onclick="commentDelete(${comment.idx}, ${qna.idx})" ><small>댓글 삭제</small></a>					
				</c:if>				
				</p>
			</td>
		...
		<tr>
			...
			<td class="content">
				<form action="commentWrite.do" method="post">					
					...
					<input type="hidden" id="linkedArticleNum" name="linkedArticleNum" value="${qna.idx}" />
